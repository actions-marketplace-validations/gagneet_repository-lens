...
| Generated documentation | `docs generate`: OpenAPI 3.1 for the scanned application's routes with unresolved schemas listed as gaps, a Mermaid ER diagram from SQL DDL and Prisma, an architecture flowchart, and a mindmap page and context pack per route area; `featuretrace propose`: draft FeatureTrace markers and fact-only JSDoc as a reviewable patch, written only with `--apply` | Business purpose, authorization and undeclared request/response shapes are not inferred; route areas are not business features; see [docs/documenting.md](docs/documenting.md) |
...
- [docs/documenting.md](docs/documenting.md) — documenting an undocumented application: generated OpenAPI, schema and maps, proposed markers and JSDoc
