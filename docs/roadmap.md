...
| 11 | this repository's domain layer imported `fastapi` and `bson` | the backend fix: `invoice_lifecycle` is pure and its Mongo-backed `transition_invoice` moved to `services/invoice_lifecycle_service.py`. `period_lifecycle` raises `PeriodNotOpenError`, which the router maps to 409. `tests/backend/test_domain_layer_purity.py` fails on any web, database or service import under `backend/domain/`, and the `[docs.python] exclude` is gone |
