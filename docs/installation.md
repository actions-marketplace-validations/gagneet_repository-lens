...
- A grep of the package for that repository's vocabulary finds three kinds of hit:
  ...
  - one SQL remedy that names `SET app.tenant_id`, that repository's RLS idiom, which
    should become generic advice;
  - `tenant_id` in the default list of identity parameters.
...
- The `scripts/validation/*` and `scripts/git/*` shims. CI, deploy preflight, the git
  hooks and the tests call them by those names.
- The backend suites that assert that repository's policy, not the package's:
  `test_every_validation_gate_is_reachable`, `test_generated_artifact_merges`,
  `test_canonical_owners` and `test_featuretrace_ratchet`. They keep running against the
  installed version.
